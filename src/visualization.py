...
class MultiRunVisualizer:
    """
    Visualizations for comparing multiple simulation runs.
    
    This class creates comparative plots showing trends and patterns
    across multiple independent simulation runs.
    """
    ...
    def plot_run_comparison(self, aggregated_stats: Dict[str, Any], 
                          output_path: Path,
                          filename: str = "run_comparison.png") -> None:
        """
        Create comparison plots across multiple runs.
        
        Args:
            aggregated_stats: Aggregated statistics from multiple runs
            output_path: Directory to save the plot
            filename: Name of the output file
        """
        fig, axes = plt.subplots(2, 2, figsize=(15, 12))
        
        # Plot 1: Fitness improvement by model
        self._plot_fitness_improvement_comparison(axes[0, 0], aggregated_stats)
        
        # Plot 2: Parent-offspring correlation by model
        self._plot_correlation_comparison(axes[0, 1], aggregated_stats)
        
        # Plot 3: Genomic distance effects
        self._plot_distance_effects_comparison(axes[1, 0], aggregated_stats)
        
        # Plot 4: Overall model performance
        self._plot_model_performance_summary(axes[1, 1], aggregated_stats)
        
        plt.tight_layout()
        self._save_figure(fig, output_path, filename)
    
    def _plot_fitness_improvement_comparison(self, ax, stats: Dict[str, Any]) -> None:
        """Plot fitness improvement comparison across models."""
        diploid_stats = stats.get("diploid_models", {})
        models = list(diploid_stats.keys())
        
        if not models:
            ax.text(0.5, 0.5, 'No diploid data', ha='center', va='center', transform=ax.transAxes)
            return
        ...
        for model in models:
            model_data = diploid_stats[model]
            if "fitness_improvement" in model_data:
                improvements.append(model_data["fitness_improvement"]["mean"])
                errors.append(model_data["fitness_improvement"]["std"])
            else:
                improvements.append(0)
                errors.append(0)
        
        ax.bar(models, improvements, yerr=errors, capsize=5, alpha=0.7)
        ax.set_title('Fitness Improvement by Model')
        ax.set_ylabel('Mean Fitness Improvement')
        ax.tick_params(axis='x', rotation=45)
    
    def _plot_correlation_comparison(self, ax, stats: Dict[str, Any]) -> None:
        """Plot parent-offspring correlation comparison."""
        diploid_stats = stats.get("diploid_models", {})
        models = list(diploid_stats.keys())
        ...
        for model in models:
            model_data = diploid_stats[model]
            if "parent_r2" in model_data:
                correlations.append(model_data["parent_r2"]["mean"])
                errors.append(model_data["parent_r2"]["std"])
            else:
                correlations.append(0)
                errors.append(0)
        
        ax.bar(models, correlations, yerr=errors, capsize=5, alpha=0.7, color='orange')
        ax.set_title('Parent-Offspring Fitness Correlation')
        ax.set_ylabel('Mean R²')
        ax.tick_params(axis='x', rotation=45)
    
    def _plot_distance_effects_comparison(self, ax, stats: Dict[str, Any]) -> None:
        """Plot genomic distance effects comparison."""
        diploid_stats = stats.get("diploid_models", {})
        models = list(diploid_stats.keys())
        ...
        for model in models:
            model_data = diploid_stats[model]
            if "avg_genomic_distance" in model_data:
                distances.append(model_data["avg_genomic_distance"]["mean"])
                errors.append(model_data["avg_genomic_distance"]["std"])
            else:
                distances.append(0)
                errors.append(0)
        
        ax.bar(models, distances, yerr=errors, capsize=5, alpha=0.7, color='green')
        ax.set_title('Average Genomic Distance')
        ax.set_ylabel('Mean Genomic Distance')
        ax.tick_params(axis='x', rotation=45)
    
    def _plot_model_performance_summary(self, ax, stats: Dict[str, Any]) -> None:
        """Create a summary performance plot."""
        ax.text(0.5, 0.5, f"Analyzed {stats.get('num_runs', 0)} runs\n\n" +
                "See individual plots for detailed comparisons", 
    ...
    def _save_figure(self, fig, output_path: Path, filename: str) -> None:
        ...
        self.logger.info(f"Saved multi-run plot: {filepath}")